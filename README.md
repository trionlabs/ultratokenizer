...
| packages/institution                             | Node/SQLite stable-right registry, exact allocation accounting and a separately pinned RPC settlement bridge                                     | Local SQLite and synthetic RPC checks; actual deployment/provider admission remains open |
...
4. Complete ATS provisioning/admission and exercise the selected backend on Hedera with fees, exact mint/transfer, rollback and invalid proofs. The normal-source profile builds locally, but its 31,759-byte creation input exceeds the documented inline creation-data limit; HFS builders, a sequential per-creation controller, bounded funding and exact readback now exist in `packages/hedera-provisioning`. Whole-graph deployment planning, aggregate funding/admission and live acceptance remain open. Native HTS association is a separate backend requirement. Record deployed identities and transactions.
