...
| Web                                                           | `npm run check:web`, `npm run build:web`; web browser scripts                            | `validate`, `browser-verification`     | Static application and browser flows with synthetic transports                             |
| Observation worker                                            | `npm run check:workers`, `npm run build:workers`                                         | `validate`                             | Actual local workerd; dry-run build, no deployment                                         |
| Hedera provisioning (separate npm lock)                       | `npm run check:hedera-provisioning`, `npm run check:ats:hfs`                             | `validate`, `ats-acceptance`           | Offline SDK transport and actual ATS input hydration, no live HFS send                     |
