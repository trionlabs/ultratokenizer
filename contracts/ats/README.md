...
Current [Hedera EthereumTransaction documentation](https://docs.hedera.com/native/smart-contracts/ethereum-transaction) describes a 24 KB inline creation-data limit and continued support for HFS-backed `callDataFileId`. This profile exceeds that inline limit. The [HFS transport package](../../packages/hedera-provisioning/README.md) now validates the complete signed/reconstructed input and provides guarded per-operation submission and hash-bound mirror recovery. Its tests preserve this actual constructor input offline. Sequential orchestration, aggregate fee accounting, receipt compatibility and network gas/opcode checks remain separate live work. [HFS creation](https://docs.hedera.com/native/files/create) and [append](https://docs.hedera.com/native/files/append) are external transactions, not actions performed by this package. No factory-based admission, live upload/deployment, genuine proof acceptance, redemption or physical backing is established here.
